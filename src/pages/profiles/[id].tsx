import { GetStaticPropsContext, GetStaticPathsContext,InferGetStaticPropsType,NextPage } from 'next';
...
const ProfilePage:NextPage<InferGetStaticPropsType<typeof getStaticProps>> = ({id,}) => {
    ...
    const tweets = api.tweet.infiniteProfileFeed.useInfiniteQuery(
        ...
        const toggleFollow = api.prisma.toggleFollow.useMutation({onSuccess:({
    ...
    <header className='sticky top-0 z-10 flex items-center border-b bg-white px-4 py-2'>
        ...
        <div className='ml-3 flex-grow'>
            <h1 className=" ml-2 text-lg font-bold">{profile.name}</h1>
...
function FollowButton({userId,isFollowing,isLoading,onClick}:{userId:string,isLoading:boolean,isFollowing:boolean,onClick:() => void}) {
