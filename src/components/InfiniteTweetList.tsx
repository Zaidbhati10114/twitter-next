...
export function InfiniteTweetList({tweets,isError,isLoading,fetchNewTweets,hasMore=false}:InfiniteTweetListProps) {
    ...
    return (
        <h2 className="my-4 text-center text-2xl text-red-500"> Oops ! Sorry ,No tweets found</h2>
    )
}
return (
    <ul>
...
function TweetCard({id,user,content,createdAt,likeCount,likedByMe}:Tweet) {
    ...
    const toggleLike = api.tweet.toggleLike.useMutation({
        onSuccess:({addedLike}) => {
            const updateData : Parameters<typeof trpcUtils.tweet.infiniteFeed.setInfinteData>[1] = (oldData) => {
        ...
        <Link href={`/profiles/${user.id}`}>
            ...
            <div className="flex flex-grow flex-col">
                <div className="flex gap-1">
                    <Link className="font-bold hover:underline focus-visible:underline" href={`/profiles/${user.id}`}>{user.name}</Link>
